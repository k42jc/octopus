...
public class ExportExcel {
    ...
    // 构造方法，传入要导出的数据
    public ExportExcel(String title, List<String> headers,
                       List<List<Object>> dataList,OutputStream out) {
    ...
    /*
     * 导出数据
     * copy from https://blog.csdn.net/m0_37556124/article/details/65444522
     *
     * 做出一点小调整
     */
    public void export() throws Exception {
        try {
            ...
            for (int i = 0; i < dataList.size(); i++) {
                ...
                HSSFRow row = sheet.createRow(i + 1);// 创建所需的行数（从第三行开始写数据）
...
//                    }
            ...
            // 让列宽随着导出的列长自动适应
            for (int colNum = 0; colNum < dataList.get(0).size(); colNum++) {
...
//                                e.printStackTrace();
                ...
                if(columnWidth >= 255){
                    columnWidth = 250;// fix The maximum column width for an individual cell is 255 characters.
                }
                if (colNum == 0) {
