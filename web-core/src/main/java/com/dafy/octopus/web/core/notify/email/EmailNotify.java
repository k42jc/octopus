...
import com.dafy.octopus.web.core.notify.NotifyStrategy;
import com.dafy.octopus.web.core.task.EmailSenderExcutor;
...
public class EmailNotify implements NotifyStrategy{
    ...
    private static Environment env = ApplicationContextProvider.getBean(Environment.class);
    private static String sender = env.getProperty("spring.mail.username","test@dafy.com");

    @Override
    public void notify(Msg msg) {
        if (msg == null || !(msg instanceof Email)) {
            CommonUtils.throwException("error","非邮件类型通知消息");
        }
        Email email = (Email) msg;

        /*TaskFatory.sendEmail(email);*/
