package com.dafy.octopus.auth.exception;
...
//            resp =
    ...
    @ResponseStatus(HttpStatus.OK)
    @ResponseBody
    public Response handleException(DuplicateKeyException exception) {
        logger.error("唯一性校验失败："+exception.getMessage()/*, exception*/);
        Response resp = new Response();
        resp.setCode(Constants.ERROR);
        resp.setMsg("当前操作已存在相同数据，请检查后重新操作");
        return resp;
    }
